...
[.lead]
dbs.mem events are presented when Distributed Block Store (DBS) detects a node's container has very low memory, which may be fixed by purge unused volumes and allow up to an hour for garbage collection between the block and slice services to run. alternatively, add more nodes.
