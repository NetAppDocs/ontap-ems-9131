...
[.lead]
ffm.aggr events are presented when an aggregate attach operation is complete. The event displays the service ID of the attached Firefly Service and the generation number, which is the number of times this aggr has been unattached.
