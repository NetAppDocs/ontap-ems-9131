...
[.lead]
master.couldnt EMS events occur when the master service on a node is unable to cast a vote to update the connection status with the current cluster master.
