...
[.lead]
director.dataip events are presented when a data IP address is being reassigned from one node to another node. The output contains the node ID of both nodes as well as whether or not the operation was done forcefully.
