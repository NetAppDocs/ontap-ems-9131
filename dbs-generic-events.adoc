...
[.lead]
dbs.generic events are presented when a Distributed Block Store (DBS) cluster fault is reported as a generic fault. The DBS is responsible for managing the data that backs the FlexVols(R). Multiple faults are mapped to this one event.
