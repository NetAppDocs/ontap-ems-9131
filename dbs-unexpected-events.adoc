...
[.lead]
dbs.unexpected events are presented when a Distributed Block Store (DBS) exception occurs unexpectedly, such as the inability to analyze existing error information. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
