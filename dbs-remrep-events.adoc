...
[.lead]
dbs.remrep events are presented when the Distributed Block Store (DBS) is executing remote replication for a pair of volumes, but that it has not reached active state for 6 hours.
