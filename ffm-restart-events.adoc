...
[.lead]
ffm.restart events are presented when the Firefly manager restarts Firefly after detecting abnormal behavior. This restart avoids a core dump, which would not help address the abnormal behavior. When Firefly comes back up, aggregates will be re-attached.
