...
[.lead]
hwlicense.install events are presented when download of the hardware license fails, which may be fixed by check for command syntax errors, the correct file path, or network connection issues.
