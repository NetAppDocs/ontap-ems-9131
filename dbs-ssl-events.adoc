...
[.lead]
dbs.ssl events are presented when Distributed Block Store (DBS) detects that a Secure Socket Layer (SSL) certificate on a node is nearing expiration. No action is needed. The system resolves this issue by automatically updating the certificate.
