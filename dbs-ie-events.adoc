...
[.lead]
dbs.ie events are presented when a Distributed Block Store (DBS) internal interface exception condition occurs. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
