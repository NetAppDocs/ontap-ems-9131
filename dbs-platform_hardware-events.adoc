...
[.lead]
dbs.platform_hardware events are presented when a Distributed Block Store (DBS) platform hardware event occurs, such as a drive hardware fault. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
