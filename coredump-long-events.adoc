...
[.lead]
coredump.long events are presented when the coredump manager thread detects that a message has been queued longer than expected, as defined by an internal threshold. System features and performance are not affected.
