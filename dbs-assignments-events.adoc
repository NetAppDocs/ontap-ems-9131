...
[.lead]
dbs.assignments events are presented when a Distributed Block Store (DBS) assignments event is reported, which occurs when the distribution of block data within the cluster changes. The DBS is responsible for managing the data that backs the FlexVols(R).
