...
[.lead]
dbs.bulk events are presented when a Distributed Block Store (DBS) event related to bulk volume operations such as backups, restores, making of Snapshot copies, or cloning occurs. If the original distributed block store event specifies multiple drives, then an EMS event is generated for each drive.
