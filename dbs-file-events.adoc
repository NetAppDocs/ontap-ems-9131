...
[.lead]
dbs.file events are presented when the Distributed Block Store (DBS) detects the mounted container file system ("/", "/persist", "/var/log") has capacity below 5 percent, which may be fixed by add additional filesystem capacity to the node or remove any unneeded files until the fault is cleared.
