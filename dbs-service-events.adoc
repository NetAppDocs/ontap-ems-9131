...
[.lead]
dbs.service events are presented when a Distributed Block Store (DBS) service event occurs, such as inability to start the FireStorm service, which may be fixed by verify that the service is started within 10 minutes.
