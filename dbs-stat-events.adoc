...
[.lead]
dbs.stat events are presented when a Distributed Block Store (DBS) statistics event occurs, such as inconsistent statistics. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
