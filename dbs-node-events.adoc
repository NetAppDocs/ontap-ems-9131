...
[.lead]
dbs.node events are presented when Distributed Block Store (DBS) detects a node with a maintenance mode failure, which may be fixed by contact netapp technical support.
