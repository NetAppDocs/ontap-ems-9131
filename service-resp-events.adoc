...
[.lead]
service.resp events are presented when a service (Firefly, Slice Service, etc.) is detected as responsive and this state is committed into the Zookeeper database. The service ID and node ID on which it is running are displayed in the event notification.
