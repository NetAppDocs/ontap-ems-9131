...
[.lead]
dbs.ensemble events are presented when a Distributed Block Store (DBS) event related to the database ensemble occurs, such as the inability to reach the database in a node, which may be fixed by verify power for all nodes and their network connectivity.
