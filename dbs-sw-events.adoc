...
[.lead]
dbs.sw events are presented when a Distributed Block Store (DBS) event relating to Software Encryption At Rest occurs, such as master key rekey completes or fails, rewrapping drive keys with new SEAR master key takes too long occur.
