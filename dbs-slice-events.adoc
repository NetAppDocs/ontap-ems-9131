...
[.lead]
dbs.slice events are presented when a Distributed Block Store (DBS) event relating to a slice service operation such as removing a metadata drive, slice reassignment i.e.
