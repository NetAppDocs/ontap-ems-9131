...
[.lead]
dbs.clone events are presented when a Distributed Block Store (DBS) event related to volume cloning occurs. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
