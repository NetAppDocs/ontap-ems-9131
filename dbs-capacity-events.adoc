...
[.lead]
dbs.capacity events are presented when the Distributed Block Store (DBS) detects that more than half of the cluster capacity is available on only one node.
