...
[.lead]
dbs.remote events are presented when a Distributed Block Store (DBS) event such as a change in cluster pair connectivity status occurs. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive, which may be fixed by free target space.
