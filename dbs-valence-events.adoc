...
[.lead]
dbs.valence events are presented when a Distributed Block Store (DBS) valence test failure event occurs. Valence test are internal periodic tests. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
