...
[.lead]
dbs.maintenance events are presented when a Distributed Block Store (DBS) maintenance mode event occurs, such as that a node no longer exists. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
