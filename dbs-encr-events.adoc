...
[.lead]
dbs.encr events are presented when Distributed Block Store (DBS) enables or disables encryption at rest on a self-encrypting drive. Encrypting drives automatically encrypt and decrypt the data as it is written or read from the drive media.
