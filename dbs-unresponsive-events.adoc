...
[.lead]
dbs.unresponsive events are presented when Distributed Block Store (DBS) Master Service detects an unresponsive service, which may be fixed by wait 10 minutes and check that the fault status has changed to resolved. if the condition persists, contact netapp technical support.
