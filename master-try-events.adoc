...
[.lead]
master.try events are presented when the associated node can not connect to current cluster master and tries to demote it by making associated global database change.
