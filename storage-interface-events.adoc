...
[.lead]
storage.interface events are presented when a storage interface reports a link down, which may be fixed by check the system and attempt to bring the network interface back online.
