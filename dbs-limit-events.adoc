...
[.lead]
dbs.limit events are presented when a Distributed Block Store (DBS) limit event occurs, such as reaching the recommended or maximum number of volumes or virtual volumes. If the original DBS event specifies multiple drives, then an EMS event is generated for each drive.
