...
[.lead]
director.aggr events are presented when an aggregate is being reassigned from one node to another node. The output contains the Firefly ID and node ID of both nodes as well as whether or not the operation was done forcefully.
