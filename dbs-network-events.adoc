...
[.lead]
dbs.network events are presented when a distributed block store detects network configuration or connectivity errors on nodes in the cluster. Multiple error conditions are mapped to this one event.
