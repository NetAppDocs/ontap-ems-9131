...
[.lead]
flexcache.autogrow events are presented when automatically growing a FlexCache volume fails because the total cluster-wide FlexCache provisioned space has exceeded the licensed limit, which may be fixed by to increase the licensed capacity, install a new license in the cluster.
